...
def _observables(posterior=False):
    ...
    fig, axes = plt.subplots(
        nrows=len(systems), ncols=1,
        figsize=figsize(.6, aspect=3),
        sharex=True
    )
    ...
    for system, color, ax in zip(systems, colors.values(), axes):
        for plot in plots:
...
def find_map():
    ...
    plot = _observables_plots()[0]

    fig, (ax, ratio_ax) = plt.subplots(
        nrows=2, ncols=1,
        figsize=figsize(1.1, aspect=.8),
        gridspec_kw=dict(
            height_ratios=[3, 1]
        )
    )

    for color, system in zip(colors.values(), systems):
        projectiles, beam_energy = parse_system(system)
        label = '{}-{} {} GeV'.format(*projectiles, beam_energy)
        for obs, subobs, opts in plot['subplots']:
            x = model.data[system][obs][subobs]['x']
            y = pred[system][obs][subobs][0]

            if system in ['UU193', 'He3Au200']:
                y *= 2
                ax.annotate(r'$\times 2$', xy=(x[-1] + 1, y[-1]),
...
@plot
def pca_vectors_variance(system='AuAu200'):
    """
    ...
    for n, (pc, var) in enumerate(zip(
            pca.components_[:5], pca.explained_variance_ratio_
    ), start=1):
...
#@plot
def validation_all(system='AuAu200'):
    """
...
@plot
def correlation_matrices(system='AuAu200'):
    """
